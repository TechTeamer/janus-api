...
class VideoRoomListenerJanusPlugin extends JanusPlugin {
  constructor (roomId, janusRoomId, roomMemberPrivateId, mediaOptions, remoteFeedId, filterDirectCandidates = false) {
    if (!roomId) {
    ...
    return new Promise((resolve, reject) => {
      this.transaction('message', {body: join}, 'event').then((param) => {
        ...
        if (!data || data.videoroom !== 'attached') {
          serviceContainer.logger.error('VideoRoomListenerJanusPlugin join answer is not attached', data, json)
          throw new Error('VideoRoomListenerJanusPlugin join answer is not attached')
        }
        if (!json.jsep) {
          serviceContainer.logger.error('VideoRoomListenerJanusPlugin join answer does not contains jsep', data, json)
    ...
    return new Promise((resolve, reject) => {
      ...
      this.transaction('message', {body: ans, jsep}, 'event').then((param) => {
        ...
        if (!data || data.started !== 'ok') {
          serviceContainer.logger.error('VideoRoomListenerJanusPlugin set answer is not ok', data, json)
